function b = rdcoordb(infilename)
...
% pattern only
if (num_items == 2)     
    b = sparse(T(:,1), T(:,2), zeros(nz,1) , m , n);
...
elseif (num_items == 3)
	if (max(abs(T(:,3))) == 0.0 )		% really a pattern matrix?
		...
		b = sparse(T(:,1), T(:,2), ones(nz,1), m , n);
	else
    	b = sparse(T(:,1), T(:,2), T(:,3), m , n);
    end;

% complex values
elseif (num_items == 4)
    b = sparse(T(:,1), T(:,2), T(:,3) + T(:,4)*sqrt(-1), m , n);
end
b=full(b);
display(b);